...
export default function LoginForm({ onLogin, onSignupToggle }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulate API call
    setTimeout(() => {
      onLogin({ email, name: email.split('@')[0], avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${email}` });
      setLoading(false);
    }, 1500);
  };
    ...
    <form className="auth-form" onSubmit={handleSubmit}>
      ...
      <div className="auth-input-group">
        <div className="flex justify-between">
          ...
          <a href="#" className="auth-link text-xs">Forgot?</a>
        </div>
        <div className="auth-input-wrapper">
      ...
      <p className="text-center text-xs text-muted mt-6">
        Don't have an account? <button type="button" onClick={onSignupToggle} className="auth-link font-bold">Sign Up</button>
      </p>
    </form>
  );
}
